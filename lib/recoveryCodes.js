...
const ivSize = 12;
const tagSize = 16;
const validChars = crypto.base32Alphabet;

exports.getValidChars = () => validChars;
...
function getCode(site, name, revision)
{
  ...
  }).then(password =>
  {
    ...
    let passwordLen = new TextEncoder().encode(password).length;
    ...
    return Promise.all([
      ...
      masterPassword.encrypt(password, undefined, false),
      passwordLen
    ]);
...
{
  return recoveryCode.replace(new RegExp(`[^${validChars}]`, "gi"), "")
                     .replace(/\w{24}/g, "$&\n")
                     .replace(/\w{12}(?=\w)/g, "$&:")
                     .replace(/\w{4}(?=\w)/g, "$&-");
}
...
function isValid(recoveryCode)
{
  return Promise.resolve().then(() =>
  {
    let buffer = crypto.fromBase32(recoveryCode);
    ...
    for (let i = 0; i < blocks; i++)
    {
      ...
      if (i == blocks - 1 && buffer[blockStart + blockSize] == crypto.pearsonHash(buffer, blockStart, blockSize, 255 - i))
        return "ok";
      if (buffer[blockStart + blockSize] != crypto.pearsonHash(buffer, blockStart, blockSize, i))
...
function decodeCode(recoveryCode)
{
  ...
  }).then(validationResult =>
  {
    ...
    let salt = crypto.toBase64(buffer.slice(pos, pos += saltSize));
    let iv = crypto.toBase64(buffer.slice(pos, pos += ivSize));
    let ciphertext = crypto.toBase64(buffer.slice(pos));

    return Promise.all([
      `${iv}_${ciphertext}`,
      masterPassword.deriveKey(salt)
    ]);
  }).then(([encrypted, key]) =>
  {
