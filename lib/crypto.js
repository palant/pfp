...
/* global crypto, TextEncoder, TextDecoder, atob, btoa */
...
function pbkdf2Derive(password, salt, length)
{
  ...
  function F(key, i)
  {
    let result = new Uint8Array(DIGEST_LENGTH);
    let iterations = NUM_ITERATIONS;

    // Initial buffer is our salt and i as 32-bit big-endian number.
    let buffer = encoder.encode(salt + "    ");
    buffer[buffer.length - 4] = (i >>> 24) & 0xFF;
    buffer[buffer.length - 3] = (i >>> 16) & 0xFF;
    buffer[buffer.length - 2] = (i >>> 8) & 0xFF;
    buffer[buffer.length - 1] = (i >>> 0) & 0xFF;
  ...
  }).then(chunks =>
  {
    ...
    for (let i = 0; i < length; i++)
      result[i] = chunks[i / DIGEST_LENGTH | 0][i % DIGEST_LENGTH];
    return result;
  });
}
...
exports.derivePassword = function(params)
{
  ...
  {
    // Edge supports WebCrypto but not PBKDF2, do things "manually".
    return pbkdf2Derive(params.masterPassword, salt, params.length);
  }).then(buffer =>
  {
