...
function deriveBits(password, salt, length)
{
  ...
  {
    if (!scryptWorker)
      scryptWorker = new Worker(require("./browserAPI").runtime.getURL("data/scrypt.js"));
...
exports.deriveKey = function(params)
{
  return Promise.resolve().then(() =>
  {
    return deriveBits(params.masterPassword, params.salt, AES_KEY_SIZE);
  }).then(array =>
  {
...
exports.encryptData = function(keyData, plaintext)
{
  return Promise.resolve().then(() =>
  {
    return crypto.subtle.importKey(
      "raw", fromBase64(keyData), "AES-GCM", false, ["encrypt"]
    );
  }).then(key =>
  {
...
exports.decryptData = function(keyData, ciphertext)
{
  return Promise.resolve().then(() =>
  {
    return crypto.subtle.importKey(
      "raw", fromBase64(keyData), "AES-GCM", false, ["decrypt"]
    );
  }).then(key =>
  {
