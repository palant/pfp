...
function parseLastpassCSV(fileContents)
{
  ...
  // Drop the header, we don't need it now that we know it's there.
  fileContents = fileContents.substring(fileContents.indexOf("\n"));

  // FIXME - We need to decode HTML entities like &amp;.
  ...
  for (let entry of parseCSV(fileContents))
  {
    let [url, username, password, extra, name, grouping, fav] = entry;
