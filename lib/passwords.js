...
let {derivePassword, derivePasswordLegacy, encryptPasswordLegacy, decryptPasswordLegacy} = require("./crypto");
...
function transformPasswordData(key, data)
{
  ...
  if (data.type == "generated2" || data.type == "generated")
  {
    let name = key;
    let revision = "";
...
function getPassword(site, name, revision)
{
  return _getSiteData(site).then(siteData =>
  {
    ...
    else if (passwordData.type == "generated")
    {
      let {length, lower, upper, number, symbol} = passwordData;
      let params = {
        masterPassword: master,
      ...
      return derivePasswordLegacy(params).then(([password, name]) => password);
    }
    else if (passwordData.type == "stored")
    {
      ...
      return decryptPasswordLegacy(params);
    }
    else
...
function checkImportedPassword(passwordData)
{
  ...
  if (passwordData.type == "generated2" || passwordData.type == "generated" || passwordData.type == "pbkdf2-sha1-generated")
  {
    ...
    let type = passwordData.type;
    if (type == "pbkdf2-sha1-generated")
      type = "generated";
    ...
    if (typeof passwordData.notes == "string")
      result.notes = passwordData.notes;
    return result;
  }
...
function importPasswordData(data)
{
  ...
  {
    return storage.getAllByPrefix(STORAGE_PREFIX);
  }).then(sites =>
  {
    try
    {
      ...
      if (importedData.aliases && Symbol.iterator in importedData.aliases)
      {
        for (let alias of importedData.aliases)
        {
          ...
          if (!siteData || _isEmpty(siteData.passwords))
            mergeActions.push(_setSiteData(alias, {alias: site}));
        }
      }
...
function addGenerated({site, name, revision, length, lower, upper, number, symbol, legacy})
{
  return lock.acquire().then(() => _getSiteData(site)).then(siteData =>
  {
    if (!siteData.passwords)
    ...
    siteData.passwords[key] = {
      type, length, lower, upper, number, symbol
    };

    return Promise.all([siteData, _setSiteData(site, siteData)]);
  }).then(([siteData, _]) => transformPasswordList(siteData.passwords)).finally(() => lock.release());
}
exports.addGenerated = addGenerated;

function addStored({site, name, password})
{
  return lock.acquire().then(() => _getSiteData(site)).then(siteData =>
  {
    let master = masterPassword.get();
    ...
    return Promise.all([site, siteData, key, encryptPasswordLegacy(params)]);
...
function setNotes(site, name, revision, notes)
{
  return lock.acquire().then(() =>
  {
    ...
    return Promise.all([_getSiteData(site), key, encryptPasswordLegacy(params)]);
