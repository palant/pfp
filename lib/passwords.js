...
function addAlias(site, alias)
{
  ...
  }).then(hasPasswords =>
  {
    ...
    return _setSiteData(site, {site, alias});
  ...
  }).then(passwordData =>
  {
    ...
    if (passwordData.type == "generated2" || passwordData.type == "generated")
    {
      ...
      if (passwordData.type == "generated2")
        ...
        return derivePasswordLegacy(params);
    }
...
function importPasswordData(data)
{
  ...
  }).then(([saltRaw, hmacSecretRaw]) =>
  {
    if (saltRaw != data.data[masterPassword.saltKey] || hmacSecretRaw != data.data[masterPassword.hmacSecretKey])
      throw "backup-from-different-instance";

    let mergeActions = [];
    for (let key in data.data)
    {
      if (key.startsWith(STORAGE_PREFIX))
        mergeActions.push(storage.set(key, data.data[key], null));
    }
...
function addStored({site, name, password})
{
  return lock.acquire().then(() => _ensureSiteData(site)).then(() =>
  {
    return _getPasswordKey(site, name);
  }).then(key =>
  {
    return Promise.all([key, storage.has(key)]);
  }).then(([key, exists]) =>
