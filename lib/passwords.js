...
let {parseLastpassCSV} = require("./lastpassImport");
...
function importPasswordData(data)
{
  ...
  }).then(sites =>
  {
    ...
    catch (parsingJSONException)
    {
      try
      {
        // Perhaps it's a LastPass CSV?
        data = parseLastpassCSV(data);

        // Guess not, so let's log why JSON parsing failed.
        if (!data)
          console.error(parsingJSONException);

        // FIXME - Why does nothing show up after the import was apparently
        //         successful?
      }
      catch (parsingLastpassCSVException)
      {
        // It looked like a LastPass CSV but we failed to parse it. Display
        // some feedback for the user, perhaps they can fix the problem.
        throw parsingLastpassCSVException;
      }
    }
