...
function importUnchanged(data)
{
  let mergeActions = [];
  for (let key in data)
  {
    if (key.startsWith(STORAGE_PREFIX))
      mergeActions.push(storage.set(key, data[key], null));
  }
  return Promise.all(mergeActions);
}

function decryptThenImport(data)
{
  ...
  }).then(entries =>
  {
    ...
    {
      if (!entry.type)
        mergeActions.push(_setSiteData(entry.site, entry));
      else if (entry.type == "generated2" || entry.type == "generated" || entry.type == "stored")
      {
        mergeActions.push(
          _getPasswordKey(entry.site, entry.name, entry.revision).then(key =>
          {
...
function importPasswordData(data)
{
  ...
  }).then(([saltRaw, hmacSecretRaw]) =>
  {
    if (saltRaw == data.data[masterPassword.saltKey] && hmacSecretRaw == data.data[masterPassword.hmacSecretKey])
    {
      ...
      return importUnchanged(data.data);
    }
    else
      return decryptThenImport(data.data);
  }).finally(() => lock.release());
}
