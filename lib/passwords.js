...
function addGenerated({site, name, revision, length, lower, upper, number, symbol, notes}, replaceExisting)
{
...
function addStored({site, name, revision, password, notes})
{
  return lock.acquire().then(() => _ensureSiteData(site)).then(() =>
  {
    return _getPasswordKey(site, name, revision);
...
function migrateData(masterPassword)
{
  if (!migrationInProgress)
  {
    ...
    {
      return require("./migration").migrateData(masterPassword, STORAGE_PREFIX, setPassword);
    }).finally(() =>
    {
