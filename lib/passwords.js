...
function getGeneratedPassword(site, passwordData)
{
  let master = masterPassword.get();
  if (!master)
    throw "master-password-required";
  let {name, revision, length, lower, upper, number, symbol} = passwordData;
  let params = {
...
function getPassword(site, name, revision)
{
  return _getSiteData(site).then(siteData =>
  {
    ...
    let passwordData = transformPasswordData(key, siteData.passwords[key]);
    if (passwordData.type == "generated")
    {
      return getGeneratedPassword(site, passwordData);
    }
