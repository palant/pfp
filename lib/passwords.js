...
import storage, {nameToStorageKey, saltKey, hmacSecretKey} from "./storage";
...
export function importPasswordData(data, masterPass)
{
  ...
  function tryNext()
  {
    let importer = importers[currentIndex++];
    return importer(data, setRaw, setSite, setPassword, masterPass).catch(e =>
    {
      if (e != "unknown_data_format" || currentIndex >= importers.length)
        throw e;
      return tryNext();
    });
  }
