...
let engine = {
  ...
  setup(provider, token, username)
  {
    ...
    }).then(() =>
    {
      ...
      triggerModificationListeners();
      this._start();
      masterPasswordEvents.on("changingPassword", this._changingPassword);
      return storage.set(this.storageKey, this.data, null);
  ...
  disable(noLock)
  {
    ...
    return (noLock ? Promise.resolve() : lock.acquire()).then(() =>
    {
      ...
      this._stop();
      triggerModificationListeners();
      masterPasswordEvents.off("changingPassword", this._changingPassword);
      return Promise.all([
...
export function addSyncModificationListener(listener)
{
  modificationListeners.push(listener);
}

export function removeSyncModificationListener(listener)
{
