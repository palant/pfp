...
let engine = {
  ...
  _start()
  {
    ...
    browser.alarms.onAlarm.addListener(this._check);
    browser.alarms.create("sync", {periodInMinutes: this.checkInterval});
    this._check({name: "sync"});
  },

  _stop()
  {
    browser.alarms.onAlarm.removeListener(this._check);
