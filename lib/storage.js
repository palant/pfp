...
import {decrypt, encrypt} from "./masterPassword";
...
function delete_(name)
{
  return browser.storage.local.remove(name).then(() =>
  {
    if (Array.isArray(name))
      return Promise.all(name.map(n => triggerModificationListeners(n)));
    else
      return triggerModificationListeners(name);
  });
}
...
{
  return Promise.all(modificationListeners.map(listener => listener(key)));
}
...
export default {
  ...
  clear, addModificationListener, removeModificationListener
};
