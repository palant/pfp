...
async function _suspendAutoLock()
{
  await browser.alarms.clear("autolock");
}

export async function suspendAutoLock()
{
  await _suspendAutoLock();
  await browser.storage.session.set({autoLockSuspended: true});
}
...
async function _resumeAutoLock()
{
  ...
  if (autolock)
  {
    autolock_delay = parseInt(autolock_delay, 10);
    if (autolock_delay <= 0)
...
export async function resumeAutoLock()
{
  await _resumeAutoLock();
  await browser.storage.session.remove("autoLockSuspended");
}
...
prefs.on("autolock", async(name, value) =>
{
  if (value)
  {
    let {autoLockSuspended} = browser.storage.session.get("autoLockSuspended");
