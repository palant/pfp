...
let {derivePassword, derivePasswordLegacy} = require("./crypto");
...
function checkPassword(masterPassword)
{
  return storage.get("masterPassword").then(value =>
  {
    ...
    return Promise.all([derivePassword(params), derivePasswordLegacy(params), hash]);
  }).then(([[hash, salt], [hashLegacy, saltLegacy], expected]) =>
  {
    if (hash == expected || hashLegacy == expected)
      rememberedMaster = masterPassword;
    else
      throw "declined";
  });
