...
let {getPasswords, addAlias} = passwords;
...
module.exports = function(panel)
{
  ...
  {
    addAlias(site, alias);
    let [newSite, passwords] = getPasswords(getCurrentHost());
    panel.port.emit("setPasswords", {site: newSite, passwords});
  });

  panel.port.on("getPasswords", function(host)
  {
    let [site, passwords] = getPasswords(host);
    panel.port.emit("setPasswords", {site, passwords});
  });
};
