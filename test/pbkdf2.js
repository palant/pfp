...
import {pbkdf2Worker as worker, deriveBitsLegacy} from "../lib/crypto.js";
...
describe("pbkdf2.js", () =>
{
  before(async function()
  {
    ...
    await deriveBitsLegacy("foobar", "asdf", 2);
  });

  async function compare(password, salt, iterations, length, expected)
  {
    let result = await pbkdf2(password, salt, iterations, length);
    expected = expected.trim().split(/\s+/).map(n => String(parseInt(n, 16)));
    expect(new Uint8Array(result).join(",").split(",")).to.be.deep.equal(expected);
  }

  it("should produce expected results for RFC6070 vectors", async function()
  {
    // See https://tools.ietf.org/html/rfc6070
    await compare("password", "salt", 1, 20, `
      0c 60 c8 0f 96 1f 0e 71
      f3 a9 b5 24 af 60 12 06
      2f e0 37 a6
    `);

    await compare("password", "salt", 2, 20, `
      ea 6c 01 4d c7 2d 6f 8c
      cd 1e d9 2a ce 1d 41 f0
      d8 de 89 57
    `);

    await compare("password", "salt", 4096, 20, `
...
/*
    await compare("password", "salt", 16777216, 20, `
...
*/
    ...
    await compare("passwordPASSWORDpassword", "saltSALTsaltSALTsaltSALTsaltSALTsalt", 4096, 25, `
    ...
    await compare("pass\0word", "sa\0lt", 4096, 16, `
      56 fa 6a a7 55 48 09 9d
      cc 37 d7 f0 34 25 e0 c3
    `);
  });

  it("should be able to handle long password and salt values", async function()
  {
    await compare(
